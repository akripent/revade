...
revade.client.on('messageCreate', async message => {
    ...
    // COMPLIMENTS
        ...
        const rulesEmbed = new EmbedBuilder()
            .setColor('#b4ace7')
            .setDescription("The **" + syst.serverOwner + "** Discord Server is a safe place for people who are __13 or older__. We would like to thank you for visiting our rule page as this helps our server to stay clean.\n\n If you notice someone who is violating the rules, please contact us by pinging one of our Discord Moderators. We appreciate your contribution!\n\n Here are the rules:")
            .addFields(
        ...
        const rules2Embed = new EmbedBuilder()
            .setColor('#b4ace7')
            .setDescription("Final Verdict lies with the " + syst.serverOwner + " Team 24/7! Please follow Discord ToS so we can keep this platform safe! https://discord.com/terms")
        ...
        const rules3Embed = new EmbedBuilder()
            .setColor('#b4ace7')
            .addFields(
                { name: '👓', value: 'This is the Official ' + syst.serverOwner + ' Discord Server! Cool right?' },
	...
	// ------- END For history ------------- //
        ...
        message.reply('...you do know you can use the help command... right?')
    }

    else if (command === 'ver' || command == 'version') {
        message.reply('Currently on `Revade 10 Valletta`')
    }
    ...
    if (command === 'stats' || command === 'statistics') {
        ...
        if (message.mentions.users.first() != undefined) {
            ...
            if (message.mentions.users.first().id == message.author.id) {
                return message.channel.send("You cannot `" + syst.prefix + "rep` yourself!")
            }
                ...
                if (fs.existsSync('reputationsystem/' + message.author.id + '/upvotes.txt')) {
                    fs.readFile('reputationsystem/' + message.author.id + '/upvotes.txt', 'utf8', (err, data) => {
                        ...
                        if (data.includes(message.mentions.users.first().id)) {
                            return message.channel.send("You already did `" + syst.prefix + "rep` on that user!")
                        }
                            ...
                            if (fs.existsSync('reputationsystem/' + message.mentions.users.first().id + '/data.txt')) {
                                ...
                                fs.readFile('reputationsystem/' + message.mentions.users.first().id + '/data.txt', 'utf8', (err, dataU) => {
                                    ...
                                    fs.writeFile('reputationsystem/' + message.mentions.users.first().id + '/data.txt', fixedrecomposed, (err) => {
                                        ...
                                        message.channel.send("Succesfully `" + syst.prefix + "rep`ed <@" + message.mentions.users.first().id + ">")
                                    });
                                });
            ...
            return message.channel.send("Invalid syntax. Do `" + syst.prefix + "rep (mention)`")
        }
